...
from inspire_hand.modbus import ModbusClient
from inspire_hand.exceptions import InspireHandError, ConnectionError, CommandError
...
class Register:
    ...
    FORCE_SET = 1498  # Force control max threshold values (12 bytes)
...
class InspireHand_melon:
    ...
    def clear_errors(self, *, verify: bool = True, retries: int = 3, delay: float = 0.2) -> bool:
        """
        Clear actuator errors via the CLEAR_ERROR register.
        Returns True if (a) verify=False and write succeeded, or (b) verify=True and all finger errors read back as 0.
        """
        ...
        for _ in range(max(1, retries)):
            ...
            # Read back errors
            try:
                errs = self.get_finger_errors()
                if all(e == 0 for e in errs):
                    ok = True
                    break
            except Exception:
                # try again on transient read failures
                pass
        return ok

    def describe_finger_errors(self) -> dict:
        """
        Read current error bytes and decode each finger's bits into names.
        Returns a dict: { 'little': [...], 'ring': [...], ..., 'thumb_rotate': [...] }
        """
        self._check_connection()
        raw = self.get_finger_errors()
        # Order matches your FingerID enum / fingers list
        names = ["little", "ring", "middle", "index", "thumb_bend", "thumb_rotate"]

        def decode(byte_val: int):
            bits = []
            if byte_val & ErrorCode.LOCKED_ROTOR:     bits.append("LOCKED_ROTOR")
            if byte_val & ErrorCode.OVER_TEMPERATURE: bits.append("OVER_TEMPERATURE")
            if byte_val & ErrorCode.OVER_CURRENT:     bits.append("OVER_CURRENT")
            if byte_val & ErrorCode.ABNORMAL_OPERATION: bits.append("ABNORMAL_OPERATION")
            if byte_val & ErrorCode.COMMUNICATION_ERROR: bits.append("COMMUNICATION_ERROR")
            return bits or ["OK"]

        return {n: decode(v) for n, v in zip(names, raw)}
    ...
    def grip(self, force: int = 700) -> None:
        """
    ...
    def stronger_no_bend(self) -> None:
        """Make a thumbs up gesture."""
        self._check_connection()

        time.sleep(0.5)
        # Close all fingers except thumb rotate
        self.little_finger.close()
        self.ring_finger.close()
        self.middle_finger.close()
        self.index_finger.close()
    ...
    def cool(self, force: int = 500) -> None:
        self._check_connection()
    ...
    def hook_4(self) -> None:
        ...
        # Close all fingers except thumb
        self.little_finger.move(500)
        self.ring_finger.move(500)
        self.middle_finger.move(500)
        self.index_finger.move(500)

    def hook_for_door(self) -> None:
        """Make a thumbs up gesture."""
        self._check_connection()
    ...
    def hook_2_1(self) -> None:
        """first part of door handle hook"""
        self._check_connection()
        ...
        # Close all fingers except thumb
        self.index_finger.move(1000)
        self.middle_finger.move(1000)
        self.little_finger.move(0)
        self.ring_finger.move(0)
        self.thumb_bend.close()

    def hook_2_2(self) -> None:
        """second part of door handle hook"""
        self._check_connection()
        ...
        # Close all fingers except thumb
        self.middle_finger.move(300)
        self.index_finger.move(300)

    def little_hook(self, force: int = 500) -> None:
        """
        Make a hook using little finger
        Args:
            force: Force threshold (0-1000)
        """
        self._check_connection()
    ...
    def thumb_front_ready(self) -> None:
        """
        thumb front
        """
        self._check_connection()
        ...
        time.sleep(1)
        # Set force thresholds
        self.thumb_rotate.move(200)

    def close_correctly(self) -> None:
        """Close all fingers (set all angles to 0)."""
        self._check_connection()
        values = [0] * 4
        self.thumb_rotate.close()
        self.modbus.write_multiple_registers(Register.ANGLE_SET, values)
        self.thumb_bend.close()

    def grasp_until_force_then_release(
            self,
            *,
            threshold: int = 600,  # 0–1000; stop when any monitored finger >= threshold
            hold_secs: float = 1.5,  # how long to hold after reaching threshold
            poll_dt: float = 0.05,  # seconds between force polls
            timeout: float = 6.0,  # safety timeout for the gripping phase
            monitor_fingers=("thumb", "index", "middle"),
            speed: int = 700,  # 0–1000; general speed cap
            force_limit: int = 400  # 0–1000; general force cap (also used for grip command)
    ):
        """
        Grips until any monitored finger's force >= threshold, holds, then releases.
        IMPORTANT: Call this *inside* an active connection:
            with hand.connect():
                grasp_until_force_then_release(hand, threshold=650, ...)
        """
        self._check_connection()
        ...
        # Configure speed/force caps for a smooth approach
        self.set_all_finger_speeds(speed)
        self.set_all_finger_forces(force_limit)

        # Start gripping
        print(f"Gripping... (monitoring {list(monitor_fingers)}, threshold={threshold})")
        self.grip(force=force_limit)

        # Helper: check if any monitored finger has reached the threshold
        def any_over_threshold() -> bool:
            # Preferred: bulk read once, then index by name
            forces = self.get_finger_forces()  # [thumb, index, middle, ring, little]
            idx = {"thumb": 0, "index": 1, "middle": 2, "ring": 3, "little": 4}
            try:
                monitored = [forces[idx[name]] for name in monitor_fingers]
            except KeyError as e:
                raise ValueError(f"Unknown finger name: {e.args[0]}. "
                                 f"Use one of {list(idx.keys())}.")
            return any(f >= threshold for f in monitored)

        # Poll until threshold or timeout
        import time
        t0 = time.time()
        while True:
            if any_over_threshold():
                print("→ Reached force threshold.")
                break
            if time.time() - t0 > timeout:
                print("→ Timeout before reaching threshold; releasing for safety.")
                break
            time.sleep(poll_dt)

        # Optional hold to stabilize
        if hold_secs > 0:
            print(f"Holding for {hold_secs:.1f}s ...")
            time.sleep(hold_secs)

        # Release
        print("Releasing (open all fingers)...")
        self.open_all_fingers()
        print("Done.")

    def toilet_stick(self) -> None:
        """Make a thumbs up gesture."""
        self._check_connection()
